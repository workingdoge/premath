...
- `tools/ci/executors/local_runner.sh` (runs `sh tools/ci/run_task.sh <task>` locally)
...
- builds an ephemeral microvm.nix flake, runs the declared `vfkit` runner, mounts workspace/output/control shares via virtiofs, runs `sh tools/ci/run_task.sh <task>` in Linux guest, writes exit code back to host
...
- run `sh tools/ci/run_task.sh <task>` in the target workspace,
