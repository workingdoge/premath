...
def _extract_failure_message(completed: subprocess.CompletedProcess[str]) -> str:
    ...
    return "required_decision_verify_invalid: required-decision-verify failed"


def _validate_payload(payload: Any) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        raise RequiredDecisionVerifyError(
            "required_decision_verify_invalid",
            "required-decision-verify payload must be an object",
        )
    if not isinstance(payload.get("errors"), list):
        raise RequiredDecisionVerifyError(
            "required_decision_verify_invalid",
            "required-decision-verify payload errors must be a list",
        )
    derived = payload.get("derived")
    if not isinstance(derived, dict):
        raise RequiredDecisionVerifyError(
            "required_decision_verify_invalid",
            "required-decision-verify payload derived must be an object",
        )
    return payload
...
def run_required_decision_verify(root: Path, verify_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if completed.returncode != 0 and cli_prefix and Path(cli_prefix[0]).name == "premath":
            ...
            if "unrecognized subcommand 'required-decision-verify'" in stderr:
        ...
        if completed.returncode != 0:
            ...
            if match:
                ...
                raise RequiredDecisionVerifyError(failure_class, reason)
            raise RequiredDecisionVerifyError("required_decision_verify_invalid", message)
        ...
        try:
            ...
            raise RequiredDecisionVerifyError(
                "required_decision_verify_invalid",
                "required-decision-verify returned invalid JSON",
            ) from exc
            ...
            if cli_prefix and Path(cli_prefix[0]).name == "premath":
                ...
                if completed.returncode != 0:
                    ...
                    if match:
                        failure_class = match.group("class")
                        reason = match.group("reason").strip() or message
                        raise RequiredDecisionVerifyError(failure_class, reason) from exc
                    raise RequiredDecisionVerifyError("required_decision_verify_invalid", message) from exc
                try:
                    payload = json.loads(completed.stdout)
