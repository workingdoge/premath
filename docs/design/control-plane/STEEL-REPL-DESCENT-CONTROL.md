...
The machine-enforced source for operation-routed REPL host actions is
`specs/premath/draft/CONTROL-PLANE-CONTRACT.json` under
`replHostActionBindings`. This table mirrors that contract for operators; the
drift-budget checker fails closed if a contract row loses its doctrine
operation route or if this table disagrees on the CLI/MCP binding.
...
| `issue.lease_renew` | n/a (MCP-only; no CLI surface) | `issue_lease_renew` |
| `issue.lease_release` | n/a (MCP-only; no CLI surface) | `issue_lease_release` |
...
6. `transition`: `issue.update --status closed` or `issue.discover`, then MCP
   `issue_lease_renew` / `issue_lease_release` when lease reconciliation is
   required.
