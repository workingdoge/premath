...
|---|---|---|---|---|
| CP-01 | Instruction envelope bypass / untyped execution | Unauthorized mutation/execution | `instruction_check`, `instruction_run`, mutation-policy instruction linking | Tighten policy rollout + reviewer governance (`bd-67`) |
| CP-02 | Parallel semantic surfaces drift | Contradictory control-plane truth | `coherence-check`, `docs-coherence-check`, `ci-drift-budget-check` | Continue reducing duplicate wrappers during migrations |
...
| CP-04 | Witness/projection integrity mismatch | False pass/fail claims | required witness/decision verify path, observation semantic invariance checks | Keep projection schema migration discipline (`bd-113`) |
| CP-05 | Dependency graph poisoning (cycle/self-loop) | Ready queue deadlock / hidden blockers | `dep add` cycle/self-loop rejection, `dep diagnostics`, MCP `dep_add` cycle rejection | Add stronger graph-level diagnostics and dep-type readiness policy (`bd-115`) |
...
| Schema/version and deprecation policy | In progress | `bd-113` |
| Reviewer-gated policy hardening | Pending reviewer pool | `bd-67` |
