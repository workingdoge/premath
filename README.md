...
| Site-change application | `premath site-apply` |
...
- `SITE`: operation topology and route eligibility
  (`specs/premath/contracts/DOCTRINE-SITE-INPUT.json`,
...
| World/route bindings | `specs/premath/draft/WORLD-REGISTRY.md`, `specs/premath/contracts/DOCTRINE-SITE-INPUT.json`, `specs/premath/contracts/CONTROL-PLANE-CONTRACT.json` | Core world-registry command + world-core conformance parity + runtime adapter parity checker | `cargo run --package premath-cli -- world-registry-check ... --json`; `python3 tools/conformance/run_world_core_vectors.py`; `python3 tools/conformance/check_runtime_orchestration.py --json` |
| Control-plane parity | `specs/premath/contracts/CONTROL-PLANE-CONTRACT.json`, `specs/premath/raw/PREMATH-COHERENCE.md` | Premath coherence checker | `mise run coherence-check` |
| Site-change morphisms | `specs/premath/draft/CHANGE-SITE.md`, `specs/premath/draft/CHANGE-INF.md` | Site-change apply + compose | `premath site-apply --change <path> --json` |
| Docs/spec linkage | `specs/premath/draft/SPEC-INDEX.md`, `specs/premath/raw/SPEC-TRACEABILITY.md` | Docs + traceability checks | `mise run docs-coherence-check` |
| Regression vectors | `specs/premath/raw/CONFORMANCE.md`, `specs/premath/raw/CAPABILITY-VECTORS.md` | Fixture-suite + capability vector runners | `mise run conformance-run` |
...
2. `crates/premath-cli/src/commands/world/registry_check.rs`:
...
- `specs/premath/archive/LLM-INSTRUCTION-DOCTRINE.md` — typed instruction
...
- `specs/premath/raw/CONFORMANCE.md`
- `specs/premath/raw/CAPABILITY-VECTORS.md`
...
- `crates/premath-kernel`:
  ...
  - Doctrine/control-plane contract parsing + world-descent requirement derivation.
...
- `crates/premath-transport`:
  ...
  - `premath site-apply --change <path> [--dry-run] [--json]` applies a
    `SiteChangeRequest` to `SITE-PACKAGE.json` through the `site.apply_change`
    transport action (`route.site_change` -> `world.site_change.v1`). Change
    requests are typed mutations (add/remove nodes, edges, covers, operations,
    world-route bindings) with digest-validated apply, composition, and descent
    semantics. See `specs/premath/draft/CHANGE-SITE.md`.
