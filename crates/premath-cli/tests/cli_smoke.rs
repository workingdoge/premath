...
fn doctrine_mcp_parity_check_json_smoke() {
    ...
    let registry = repo_root.join("specs/premath/contracts/DOCTRINE-OP-REGISTRY.json");
...
#[test]
fn doctrine_site_check_json_smoke() {
    let output = run_premath([
        OsString::from("doctrine-site-check"),
        OsString::from("--json"),
    ]);
    assert_success(&output);

    let payload = parse_json_stdout(&output);
    assert_eq!(payload["checkKind"], "ci.doctrine_site_check.v1");
    assert_eq!(payload["result"], "accepted");
}
...
fn spec_traceability_check_json_smoke() {
    ...
    let matrix_path = repo_root.join("specs/premath/raw/SPEC-TRACEABILITY.md");
...
#[test]
fn docs_coherence_check_json_smoke() {
    let crate_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let repo_root = crate_dir
        .parent()
        .and_then(|p| p.parent())
    ...
    let output = run_premath([
        OsString::from("docs-coherence-check"),
        OsString::from("--repo-root"),
        repo_root.as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    ...
    let payload = parse_json_stdout(&output);
    assert_eq!(payload["checkKind"], "ci.docs_coherence_check.v1");
    assert_eq!(payload["result"], "accepted");
}
...
fn transport_check_json_smoke() {
    ...
    assert_eq!(payload["result"], "accepted");
    assert_eq!(payload["failureClasses"], serde_json::json!([]));
    assert_eq!(payload["actionCount"], 13);
...
fn coherence_check_json_smoke() {
    ...
    let repo_root = crate_dir
        ...
        .expect("workspace root should be two levels above crate dir")
        .to_path_buf();
    let contract = repo_root.join("specs/premath/contracts/COHERENCE-CONTRACT.json");
...
fn coherence_check_rejects_on_coherence_spec_obligation_drift() {
    ...
    let repo_root = crate_dir
        ...
        .expect("workspace root should be two levels above crate dir")
        .to_path_buf();
    let contract_path = repo_root.join("specs/premath/contracts/COHERENCE-CONTRACT.json");
    let coherence_spec_path = repo_root.join("specs/premath/raw/PREMATH-COHERENCE.md");
...
// ── site-change builder CLI smoke tests ──────────────────────────────────────

fn write_minimal_site_package(dir: &Path) -> PathBuf {
    let pkg_dir = dir.join("specs/premath/site-packages/premath.doctrine_operation_site.v0");
    fs::create_dir_all(&pkg_dir).expect("site pkg dir should be created");
    let pkg_path = pkg_dir.join("SITE-PACKAGE.json");
    ...
    fs::write(&pkg_path, serde_json::to_string_pretty(&pkg).unwrap())
        .expect("site package should be written");
    pkg_path
}

#[test]
fn site_digest_json_smoke() {
    let tmp = TempDirGuard::new("site-digest-json");
    write_minimal_site_package(tmp.path());
    let output = run_premath([
        OsString::from("site-digest"),
        OsString::from("--repo-root"),
        tmp.path().as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    ...
    let payload = parse_json_stdout(&output);
    assert_eq!(payload["result"], "accepted");
    assert!(payload["digest"].as_str().is_some());
    assert_eq!(payload["summary"]["nodeCount"], 2);
    assert_eq!(payload["summary"]["edgeCount"], 1);
}

#[test]
fn site_build_change_json_smoke() {
    let tmp = TempDirGuard::new("site-build-json");
    write_minimal_site_package(tmp.path());
    let mutations_path = tmp.path().join("mutations.json");
    ...
    fs::write(&mutations_path, serde_json::to_string(&mutations).unwrap())
        .expect("mutations should be written");

    let output = run_premath([
        OsString::from("site-build"),
        OsString::from("--mutations"),
        mutations_path.as_os_str().to_os_string(),
        OsString::from("--repo-root"),
        tmp.path().as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    ...
    let payload = parse_json_stdout(&output);
    assert_eq!(payload["result"], "accepted");
    assert!(payload["changeRequest"].is_object());
    assert!(payload["changeRequest"]["changeId"].as_str().is_some());
    assert_eq!(payload["morphismKind"], "horizontal");
}

#[test]
fn site_compose_changes_json_smoke() {
    let tmp = TempDirGuard::new("site-compose-json");
    write_minimal_site_package(tmp.path());
    let repo_root_str = tmp.path().to_str().unwrap();

    // Build first change
    let mutations1_path = tmp.path().join("mutations1.json");
    fs::write(
        &mutations1_path,
        serde_json::to_string(&serde_json::json!([
            {"type": "addNode", "id": "n3", "path": "n3.md", "kind": "conformance", "requiresDeclaration": false}
        ]))
        .unwrap(),
    )
    .expect("mutations1 should be written");

    let build1 = run_premath([
        OsString::from("site-build"),
        OsString::from("--mutations"),
        mutations1_path.as_os_str().to_os_string(),
        OsString::from("--repo-root"),
        tmp.path().as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    assert_success(&build1);
    let b1 = parse_json_stdout(&build1);
    let r1 = &b1["changeRequest"];

    // Apply first change via transport-dispatch with explicit repoRoot
    let apply_payload = serde_json::json!({
        "changeRequest": r1,
        "dryRun": false,
        "repoRoot": repo_root_str
    });
    let apply1 = run_premath([
        OsString::from("transport-dispatch"),
        OsString::from("--action"),
        OsString::from("site.apply_change"),
        OsString::from("--payload"),
        OsString::from(serde_json::to_string(&apply_payload).unwrap()),
        OsString::from("--json"),
    ]);
    assert_success(&apply1);

    // Build second change from updated state
    let mutations2_path = tmp.path().join("mutations2.json");
    fs::write(
        &mutations2_path,
        serde_json::to_string(&serde_json::json!([
            {"type": "addNode", "id": "n4", "path": "n4.md", "kind": "conformance", "requiresDeclaration": false}
        ]))
        .unwrap(),
    )
    .expect("mutations2 should be written");

    let build2 = run_premath([
        OsString::from("site-build"),
        OsString::from("--mutations"),
        mutations2_path.as_os_str().to_os_string(),
        OsString::from("--repo-root"),
        tmp.path().as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    assert_success(&build2);
    let b2 = parse_json_stdout(&build2);
    let r2 = &b2["changeRequest"];

    // Compose r1 and r2
    let r1_path = tmp.path().join("r1.json");
    fs::write(&r1_path, serde_json::to_string_pretty(r1).unwrap()).expect("r1 should be written");
    let r2_path = tmp.path().join("r2.json");
    fs::write(&r2_path, serde_json::to_string_pretty(r2).unwrap()).expect("r2 should be written");

    let output = run_premath([
        OsString::from("site-compose"),
        OsString::from("--request1"),
        r1_path.as_os_str().to_os_string(),
        OsString::from("--request2"),
        r2_path.as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    assert_success(&output);
    let payload = parse_json_stdout(&output);
    assert_eq!(payload["result"], "accepted");
    assert!(payload["composedRequest"].is_object());
    assert_eq!(payload["composedRequest"]["fromDigest"], r1["fromDigest"]);
    assert_eq!(payload["composedRequest"]["toDigest"], r2["toDigest"]);
}

#[test]
fn site_change_chain_check_json_smoke() {
    let tmp = TempDirGuard::new("site-chain-check-json");
    write_minimal_site_package(tmp.path());

    // Should create genesis and pass (no log yet)
    let output = run_premath([
        OsString::from("site-change-chain-check"),
        OsString::from("--repo-root"),
        tmp.path().as_os_str().to_os_string(),
        OsString::from("--json"),
    ]);
    ...
    assert_eq!(payload["schema"], 1);
    assert_eq!(payload["result"], "accepted");
    assert_eq!(payload["logEntryCount"], 1);
}
