...
fn load_coherence_contract_input_paths(
    ...
    let mut base_paths = vec![
        ...
        repo_root.join("specs/premath/contracts/COHERENCE-CONTRACT.json"),
...
fn collect_topology_metrics(
    ...
    let contracts_dir = repo_root.join("specs/premath/contracts");
    let raw_dir = repo_root.join("specs/premath/raw");
    let design_dir = repo_root.join("docs/design");
    let traceability_path = raw_dir.join("SPEC-TRACEABILITY.md");
    let doctrine_site_path = contracts_dir.join("DOCTRINE-SITE.json");
...
fn build_drift_budget_payload(
    ...
    let capability_registry_path =
        repo_root.join("specs/premath/contracts/CAPABILITY-REGISTRY.json");
    let doctrine_op_registry_path =
        repo_root.join("specs/premath/contracts/DOCTRINE-OP-REGISTRY.json");
    let spec_index_path = repo_root.join("specs/premath/draft/SPEC-INDEX.md");
    let conformance_path = repo_root.join("specs/premath/raw/CONFORMANCE.md");
...
pub fn run(
    ...
    let coherence_contract_path = repo_root.join("specs/premath/contracts/COHERENCE-CONTRACT.json");
    let control_plane_contract_path =
        repo_root.join("specs/premath/contracts/CONTROL-PLANE-CONTRACT.json");
