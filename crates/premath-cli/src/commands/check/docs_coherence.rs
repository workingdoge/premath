...
const EXPECTED_DOCTRINE_CHECK_COMMANDS: [&str; 4] = [
    ...
    "cargo run --package premath-cli -- doctrine-mcp-parity-check --mcp-source crates/premath-cli/src/commands/mcp_serve.rs --registry specs/premath/contracts/DOCTRINE-OP-REGISTRY.json --json",
...
fn evaluate(repo_root: &Path) -> Result<DocsCoherenceEvaluation, String> {
    ...
    let capability_registry = repo_root.join("specs/premath/contracts/CAPABILITY-REGISTRY.json");
    let control_plane_contract =
