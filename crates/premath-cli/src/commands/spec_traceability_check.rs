...
fn collect_spec_files(dir: &Path, require_draft_status: bool) -> Result<Vec<String>, String> {
    ...
    for entry in files {
        ...
        if name == "README.md" || name == "SPEC-TRACEABILITY.md" {
            continue;
        }
        match path.extension().and_then(|ext| ext.to_str()) {
            Some("md") => {
                if require_draft_status {
            ...
            Some("json") => collected.push(name.to_string()),
            _ => {}
        }
    }
    Ok(collected)
...
fn promoted_draft_specs(draft_dir: &Path) -> Result<Vec<String>, String> {
    let mut promoted = collect_spec_files(draft_dir, true)?;
    // Also scan sibling directories for moved specs/contracts
    if let Some(parent) = draft_dir.parent() {
        for sibling in ["contracts", "raw", "archive"] {
            let sibling_dir = parent.join(sibling);
            promoted.extend(collect_spec_files(&sibling_dir, false)?);
        }
    }
    promoted.sort();
    promoted.dedup();
