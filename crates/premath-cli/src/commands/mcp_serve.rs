...
use premath_observe::ux::{ObserveQuery, SurrealObservationBackend, UxService};
use premath_observe::{ProjectionMatchMode, QueryCache};
...
impl ServerHandler for PremathMcpHandler {
    ...
    async fn handle_call_tool_request(
        ...
        match tool_params {
            ...
            PremathTools::SiteCurrentDigestTool(_tool) => call_site_current_digest(&self.config),
            PremathTools::SiteBuildChangeTool(tool) => call_site_build_change(&self.config, tool),
            PremathTools::SiteComposeChangesTool(tool) => call_site_compose_changes(tool),
            PremathTools::SiteApplyChangeTool(tool) => call_site_apply_change(&self.config, tool),
        }
    }
}
...
#[mcp_tool(
    name = "site_current_digest",
    description = "Return current SITE-PACKAGE.json canonical digest and summary counts (nodes, edges, operations, covers, world-route binding rows)",
    read_only_hint = true
)]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, Default)]
#[serde(rename_all = "camelCase")]
struct SiteCurrentDigestTool {}

#[mcp_tool(
    name = "site_build_change",
    description = "Build a valid SiteChangeRequest from a mutation list (JSON string). Auto-computes fromDigest, toDigest, morphismKind, and changeId. Validates each mutation precondition. Returns the built request — does NOT apply.",
    read_only_hint = false
)]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
struct SiteBuildChangeTool {
    /// JSON string of the mutations array
    mutations: String,
    #[serde(default)]
    #[serde(alias = "preservation_claims")]
    preservation_claims: Option<Vec<String>>,
}

#[mcp_tool(
    name = "site_compose_changes",
    description = "Compose two SiteChangeRequests (JSON strings) into one. Validates composability (r1.toDigest == r2.fromDigest) and returns the composed request.",
    read_only_hint = false
)]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
struct SiteComposeChangesTool {
    /// JSON string of the first SiteChangeRequest
    request1: String,
    /// JSON string of the second SiteChangeRequest
    request2: String,
}

#[mcp_tool(
    name = "site_apply_change",
    description = "Apply a SiteChangeRequest (JSON string) to SITE-PACKAGE.json. Use dry_run=true to validate without writing.",
    read_only_hint = false,
    idempotent_hint = false
)]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
struct SiteApplyChangeTool {
    /// JSON string of the SiteChangeRequest
    #[serde(alias = "change_request")]
...
tool_box!(
    ...
    [
        ...
        InstructionRunTool,
        SiteCurrentDigestTool,
        SiteBuildChangeTool,
        SiteComposeChangesTool,
        SiteApplyChangeTool
    ]
);
...
fn call_site_build_change(
    config: &PremathMcpConfig,
    tool: SiteBuildChangeTool,
) -> std::result::Result<CallToolResult, CallToolError> {
    let repo_root = resolve_repo_root(&config.repo_root);
    let mutations_value: Value = serde_json::from_str(&tool.mutations)
        .map_err(|e| call_tool_error(format!("failed to parse mutations JSON: {e}")))?;
    let claims = tool.preservation_claims.unwrap_or_default();
    let request = json!({
        "action": "site.build_change",
        "payload": {
            "mutations": mutations_value,
...
fn call_site_compose_changes(
    tool: SiteComposeChangesTool,
) -> std::result::Result<CallToolResult, CallToolError> {
    let r1_value: Value = serde_json::from_str(&tool.request1)
        .map_err(|e| call_tool_error(format!("failed to parse request1 JSON: {e}")))?;
    let r2_value: Value = serde_json::from_str(&tool.request2)
        .map_err(|e| call_tool_error(format!("failed to parse request2 JSON: {e}")))?;
    let request = json!({
...
fn call_site_apply_change(
    config: &PremathMcpConfig,
    tool: SiteApplyChangeTool,
) -> std::result::Result<CallToolResult, CallToolError> {
    let repo_root = resolve_repo_root(&config.repo_root);
    let change_value: Value = serde_json::from_str(&tool.change_request)
        .map_err(|e| call_tool_error(format!("failed to parse change_request JSON: {e}")))?;
    let dry_run = tool.dry_run.unwrap_or(false);
    let request = json!({
