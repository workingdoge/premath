...
| `WORLD-REGISTRY.md` | `cargo run --package premath-cli -- world-registry-check --site-input specs/premath/contracts/DOCTRINE-SITE-INPUT.json --operations specs/premath/contracts/DOCTRINE-OP-REGISTRY.json --control-plane-contract specs/premath/contracts/CONTROL-PLANE-CONTRACT.json --json`; `cargo run --package premath-cli -- runtime-orchestration-check --control-plane-contract specs/premath/contracts/CONTROL-PLANE-CONTRACT.json --doctrine-op-registry specs/premath/contracts/DOCTRINE-OP-REGISTRY.json --harness-runtime specs/premath/raw/HARNESS-RUNTIME.md --doctrine-site-input specs/premath/contracts/DOCTRINE-SITE-INPUT.json --json`; `mise run doctrine-check`; `python3 tools/conformance/run_world_core_vectors.py`; `python3 tools/conformance/run_runtime_orchestration_vectors.py` (includes constructor `route.transport.dispatch` bound/missing vectors); `mise run coherence-check` (`gate_chain_parity` world-registry iterated context-chain + Palmgren reference parity); `mise run docs-coherence-check` | covered | - |
...
| `CHANGE-INF.md` | `capabilities.change_morphisms` vectors | covered | - |
| `CHANGE-SITE.md` | `cargo run --package premath-cli -- site-apply --change bootstrap/site-apply-change.json --dry-run --json` (bootstrap self-application); `cargo test -p premath-kernel -- site_change` (kernel unit tests) | covered | - |
...
| `CAPABILITY-REGISTRY.json` | `cargo run --package premath-cli -- docs-coherence-check --repo-root . --json`; `python3 tools/conformance/run_capability_vectors.py`; `mise run coherence-check` | covered | - |
...
| `EVIDENCE-INF.md` | `mise run docs-coherence-check`; `mise run coherence-check` (`gate_chain_parity` Stage 1+Stage 2 parity/rollback/authority checks + Stage 2 direct bidir-evidence-route checks + Stage 3 typed-first closure mapping checks); `python3 tools/conformance/run_fixture_suites.py --suite coherence-contract` (`gate_chain_parity_stage1_*` + `gate_chain_parity_stage2_*` vectors); `python3 tools/conformance/run_capability_vectors.py --capability capabilities.ci_witnesses` (boundary-authority lineage + obstruction roundtrip vectors); `capabilities.unified_evidence` vectors | covered | - |
| `EVIDENCE-SITE.md` | `capabilities.unified_evidence` vectors | covered | - |
...
| `OBSERVATION-INF.md` | `capabilities.observation_semantics` vectors | covered | - |
| `OBSERVATION-SITE.md` | `capabilities.observation_semantics` vectors | covered | - |
| `SIGPI-INF.md` | `capabilities.sigpi_stepping` vectors | covered | - |
| `SIGPI-SITE.md` | `capabilities.sigpi_stepping` vectors | covered | - |
