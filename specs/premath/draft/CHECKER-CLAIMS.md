---
slug: draft
shortname: CHECKER-CLAIMS
title: workingdoge.com/premath/CHECKER-CLAIMS
name: Checker Claim Registry
status: draft
...
This document defines **checker claims** and the **canonical vector suites**
...
Checker claims are established by **running code** against canonical vectors
for the profiles described below.
...
Checker claims have both a human-readable claim string and a canonical
...
The Core claim does not imply any optional interop, control-plane, raw, or
...
claim alone. Implementations SHOULD substantiate the Kernel claim by one of:
...
suite (`tests/checker/fixtures/kernel-profile/`) for deterministic scenario
...
They are Premath-side adapters over carrier artifacts. Generic KCIR substrate
meaning belongs to `fish/sites/kcir`; Premath interop claims decide only
whether a carried artifact is acceptable for a Premath claim boundary.
...
Capabilities MUST be explicit: if a capability is not claimed, any vectors that
exercise that capability's optional branches MUST be rejected deterministically
or treated as out-of-scope for the claimed checker boundary.
...
For governance doctrine profile support, implementations MAY claim:
...
1. Parse KCIR nodes and verify all referenced payloads using the KCIR site
   substrate/profile boundary plus the transitional Premath adapter specs
   (`profile/kcir/README`, `draft/KCIR-CORE`, `draft/REF-BINDING`).
...
KCIR compatibility is necessary for KCIR-carried Premath interop claims, but it is
not sufficient for Premath acceptance. Premath acceptance additionally
requires Premath Core verdict and Gate failure-class preservation for fixed
semantic inputs.
...
Tracker mutation, issue leases, ready queues, and discover/claim workflows are
not Premath checker obligations; they belong to Tusk/downstream tracker
sites and may compile normalized work claims to Premath checker inputs.
...
### 3.6 Required behavior for instruction typing capability

If capability `capabilities.instruction_typing` is claimed, implementation MUST:

16. classify instruction handling explicitly as `typed(kind)` or
    `unknown(reason)`,
17. reject `unknown(reason)` deterministically when no explicit policy route is
    permitted, and
18. preserve kernel verdict and Gate failure classes across paired local/external
    instruction-typing profile invariance vectors,
19. ingest typed LLM proposal payloads as checking-only inputs (never authored
    synthesis inputs),
20. reject proposal payloads that are unbound to
...
- `tests/checker/fixtures/interop-core/{golden,adversarial}/`
- `tests/checker/fixtures/gate/{golden,adversarial}/`
- `tests/checker/fixtures/kernel-profile/{golden,adversarial}/`
- `tests/checker/fixtures/capabilities/<capability-id>/{golden,adversarial,invariance}/`

This repository's direct checker surface executes:
...
entrypoints remain auditable from doctrine root through runtime layers.
...
- `dm.presentation.projection` (claim/profile compatibility is architecture-agnostic)
- `dm.commitment.attest` (when commitment capabilities are claimed)
