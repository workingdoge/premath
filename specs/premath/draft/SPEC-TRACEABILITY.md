...
| `UNIFICATION-DOCTRINE.md` | `mise run docs-coherence-check`; `mise run coherence-check` (`gate_chain_parity` Stage 1+Stage 2 parity/rollback/authority checks + Stage 2 kernel sentinel checks); `python3 tools/conformance/run_fixture_suites.py --suite coherence-contract` (`gate_chain_parity_stage1_*` + `gate_chain_parity_stage2_*` vectors); `python3 tools/conformance/run_capability_vectors.py --capability capabilities.ci_witnesses` (boundary-authority lineage + obstruction roundtrip vectors); decision-log traceability via `check_spec_traceability.py` | covered | - |
