...
- keep worker-orchestration routing aligned with cover/refinement and
  Unified Evidence factoring boundaries from `draft/EVIDENCE-INF`.
...
- Unified Evidence factoring and lane ownership (`draft/EVIDENCE-INF`
  §1 and §3).
...
- harness-session operation nodes include `read`, `write`, and `bootstrap`
  paths in `crates/premath-cli/src/commands/harness/session.rs`.
