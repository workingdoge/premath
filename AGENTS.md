...
- One-shot commands:
  ...
  - `direnv exec . sh tools/ci/run_task.sh precommit`
...
- Repo task entrypoints are direct shell/Python commands:
  - task aliases via `sh tools/ci/run_task.sh <task>`
  - baseline composition in `tools/ci/baseline_tasks.json`
...
- `sh tools/ci/run_task.sh baseline` — full local closure gate (fmt + clippy + build/tests + toy + KCIR toy + conformance checks + traceability matrix check + coherence-check + drift-budget check + doctrine-site check; includes `rust-setup` for `rustfmt`/`clippy` components).
- `sh tools/ci/run_task.sh hk-install` — install optional `hk`-managed git hooks using `hk.pkl`.
- `sh tools/ci/run_task.sh hk-pre-commit` / `sh tools/ci/run_task.sh hk-pre-push` — run hk hook profiles manually.
- `sh tools/ci/run_task.sh hk-check` / `sh tools/ci/run_task.sh hk-fix` — run hk baseline check or fast local fixes (`hk-fix` runs on all files with no auto-stage).
- `sh tools/ci/run_task.sh ci-command-surface-check` — enforce direct script/Nix command-surface references (reject legacy task-runner command/file surfaces).
- `sh tools/ci/run_task.sh ci-hygiene-check` — enforce repository hygiene guardrails plus issue-graph contract checks (epic typing, active-issue acceptance/proof fields).
- `sh tools/ci/run_task.sh ci-pipeline-check` — validate provider workflow wrappers call canonical provider-neutral pipeline entrypoints via `premath pipeline-wiring-check`.
- `sh tools/ci/run_task.sh ci-pipeline-test` — run deterministic unit tests for provider-neutral pipeline summary/digest emission.
- `sh tools/ci/run_task.sh ci-observation-test` — run native deterministic reducer/query tests for `Observation Surface v0`.
- `sh tools/ci/run_task.sh ci-observation-build` — build `artifacts/observation/latest.json` + `artifacts/observation/events.jsonl` from CI witness artifacts.
- `sh tools/ci/run_task.sh ci-observation-query` — query the latest observation surface (`latest`, `needs_attention`, `instruction`, `projection`).
- `sh tools/ci/run_task.sh ci-observation-serve` — run a tiny HTTP read API over Observation Surface v0 (`GET /latest`, `/needs-attention`, `/instruction`, `/projection`).
- `sh tools/ci/run_task.sh ci-observation-check` — enforce semantic projection invariance via `premath observe-check` (observation output must equal deterministic reducer output from CI witness artifacts).
- `sh tools/ci/run_task.sh ci-drift-budget-check` — enforce deterministic drift-budget sentinels across SPEC-INDEX/CAPABILITY-REGISTRY maps, control-plane lane bindings, coherence required obligation sets, SigPi notation, and coherence-cache input closure.
- `python3 -m http.server 43173 --directory docs` — serve docs locally (includes `docs/observation/index.html` dashboard).
- `sh tools/ci/run_task.sh ci-pipeline-required` — run provider-neutral required-gate pipeline (`tools/ci/pipeline_required.py`) with deterministic retry-policy enforcement from `policies/control/harness-retry-policy-v1.json` and terminal escalation mapping to `premath issue` mutations (`issue_discover` / `mark_blocked` / `stop`).
- `sh tools/ci/run_task.sh ci-pipeline-instruction` — run provider-neutral instruction pipeline (`INSTRUCTION=instructions/<ts>-<id>.json`) with deterministic retry-policy enforcement from `policies/control/harness-retry-policy-v1.json` and terminal escalation mapping to `premath issue` mutations (`issue_discover` / `mark_blocked` / `stop`).
- `sh tools/ci/run_task.sh ci-check` — canonical gate entrypoint through `tools/ci/run_gate.sh` (SqueakSite profile switch: `PREMATH_SQUEAK_SITE_PROFILE=local|external`; legacy `PREMATH_EXECUTOR_PROFILE` still accepted).
- `sh tools/ci/run_task.sh ci-instruction` — run one instruction envelope (`INSTRUCTION=instructions/<ts>-<id>.json`) and emit `artifacts/ciwitness/<instruction-id>.json`.
...
- `sh tools/ci/run_task.sh infra-up` / `sh tools/ci/run_task.sh infra-down` — optional Terraform/OpenTofu provisioning profile for external runner binding (`tools/infra/terraform/`).
- `sh tools/ci/run_task.sh ci-check-tf` — gate execution via Terraform/OpenTofu-resolved external runner (`tools/ci/run_gate_terraform.sh`, default profile `local`).
- `sh tools/ci/run_task.sh ci-check-tf-local` — Terraform/OpenTofu path pinned to local runner profile.
- `sh tools/ci/run_task.sh ci-check-tf-microvm` — experimental/prototype Terraform/OpenTofu path using `darwin_microvm_vfkit` profile.
- `sh tools/ci/run_task.sh jj-alias-install` — install repo-local JJ aliases (`jj gate-fast|gate-fix|gate-check|gate-pre-commit`) that delegate to hk/direct script gates.
- `sh tools/ci/run_task.sh pf-start` / `sh tools/ci/run_task.sh pf-status` / `sh tools/ci/run_task.sh pf-stop` — optional pitchfork orchestration for local daemons in `pitchfork.toml` (`pf-start` starts both `docs-preview` and `observation-api`).
- `sh tools/ci/run_task.sh pf-gate-loop-start` / `sh tools/ci/run_task.sh pf-gate-loop-stop` — optional background `ci-check` loop via pitchfork (`ci-check` every 30m).
- `sh tools/ci/run_task.sh mcp-serve` — run the stdio MCP server surface over premath issue/dep/observe/doctrine tools (JSONL-authoritative memory, `instruction-linked` mutation policy).
- `sh tools/ci/run_task.sh conformance-run` — run executable fixture suites (Interop Core + Gate + Witness-ID + cross-model kernel profile + Tusk runtime contract vectors + capability vectors) through the cached suite runner.
- `sh tools/ci/run_task.sh doctrine-check` — validate doctrine declarations/site reachability, MCP doctrine-operation parity, and doctrine-inf semantic boundary vectors (`specs/premath/draft/DOCTRINE-SITE.json`, `tests/conformance/fixtures/doctrine-inf/`).
- `sh tools/ci/run_task.sh traceability-check` — validate promoted draft spec coverage matrix integrity and authority-class parity (`specs/premath/draft/SPEC-TRACEABILITY.md`, `specs/premath/AUTHORITY-MAP.json`).
- `sh tools/ci/run_task.sh coherence-check` — evaluate typed coherence obligations from `specs/premath/draft/COHERENCE-CONTRACT.json` and emit deterministic checker witness output.
- `sh tools/ci/run_task.sh precommit` — run the attested required gate chain.
- `sh tools/ci/run_task.sh conformance-check` — validate capability fixture stubs/invariance pairs.
...
- `cargo run --package premath-cli -- observe-check --repo-root . --json` — verify Observation Surface v0 projection invariance.
...
- `cargo run --package premath-cli -- issue check --issues .premath/issues.jsonl --json` — run deterministic issue-memory contract checks (epic typing + active acceptance/verification + note-size warnings + compactness drift).
...
- Forge branch-protection administration is external to the Premath checker contract; the repo-level required status check remains `ci-required`.
...
- Treat `sh tools/ci/run_task.sh baseline` as the minimum local merge gate.
- If using `hk`, keep `pre-push`/`check` mapped to the same baseline closure gate.
- For spec/conformance fixture edits, run `sh tools/ci/run_task.sh conformance-check`.
